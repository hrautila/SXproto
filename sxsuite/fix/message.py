...
from sxsuite.fix.context import *
...
class FixMessage(list):
    ...
    def split_message(self, context):
        """Split message to header and body."""
        hdr = FixMessage()
        body = FixMessage()
        for x in self:
            tagnum = int(x.split('=')[0])
            if  tagnum in context.header_ids:
                hdr.append(x)
            elif tagnum not in context.trailer_ids:
                body.append(x)
        # endfor
        return hdr, body
...
class FixObject(object):
    ...
    def to_message(self, header, full=False):
        """Translate ``FixObject`` to a ``FixMessage``."""
        hdr_m = self.mheader(header)
        msg_m = self.mbody()
        hdr_m.join(msg_m)
        if full:
    ...
    def mheader(self, header):
        return _make_mheader(self, header, header._context)

    def mbody(self):
        return _make_mbody(self, self._context)
