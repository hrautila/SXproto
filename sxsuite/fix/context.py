...
class FixContext(object):
    ...
    def __init__(self, name='', version='4.4', module_name=None):
        if not module_name:
            module_name = 'sxsuite.fix.fix%s' % version.replace('.', '')
        ...
        self.header_ids = getattr(self._fixmod, 'FIX_HEADER_FIELD_IDS')
        self.trailer_ids = getattr(self._fixmod, 'FIX_TRAILER_FIELD_IDS')
    ...
    def desc_for_name(self, name):
        ...
        return fix_desc_for_name(self._fixmod, name)
...
def fix_name_for_msgtype(fixmod, mtyp):
    return getattr(fixmod, '_fix_msgtype_table')[mtyp]

def fix_class_for_msgname(fixmod, name):
    return getattr(fixmod, name)


#def _fix_group_add(fixmod, name, num, gspec):
#    gdesc = FixGroupDescriptor(name, num)
#    fixmod._fix_field_types[name] = gdesc
#    fixmod._fix_field_numbers[num] = gdesc
#    fixmod._fix_group_types[name] = gspec
#    fixmod._fix_group_numbers[num] = fixfields._fix_group_types[name]


#def _fix_desc_add(fixmod, name, num, ftype):
#    desc = FixFieldDescriptor(name, num, ftype, fix_pytype(ftype))
#    fixmod._fix_field_types[name] = desc
#    fixmod._fix_field_numbers[num] = desc
